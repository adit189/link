...
const LOGO_URL = "Primary - Variant.jpg"; 
...
const Button = ({ children, onClick, variant = 'primary', className = '', icon: Icon, disabled, type="button" }) => {
  ...
  const variants = {
    primary: `bg-[${COLOR_PRIMARY}] text-white shadow-lg hover:shadow-xl hover:scale-[1.02]`,
    secondary: `bg-white text-[${COLOR_PRIMARY}] hover:bg-slate-50 shadow-sm`,
    danger: `bg-red-50 text-red-600 border border-red-100 hover:bg-red-100`,
    ghost: `bg-transparent text-slate-500 hover:text-[${COLOR_PRIMARY}] hover:bg-slate-50`,
    whatsapp: `bg-green-600 text-white hover:bg-green-700 shadow-xl hover:scale-105`
  };

  // Inject inline style for precise custom color handling
  const inlineStyle = variant === 'primary' ? { backgroundColor: COLOR_PRIMARY } : 
                      variant === 'secondary' ? { color: COLOR_PRIMARY, border: `1.5px solid ${COLOR_PRIMARY}` } : {};
...
const Input = ({ label, value, onChange, placeholder, type = "text", ...props }) => (
  <div className="mb-4 text-left">
    ...
    <input
      ...
      style={{ '--tw-ring-color': COLOR_PRIMARY }}
      {...props}
    />
  </div>
);
...
export default function App() {
  ...
  const [projects, setProjects] = useState([
    {
      ...
      items: [
        { id: 1, type: 'video', category: 'video', title: 'Video Cinematic 1 Menit', url: 'https://www.youtube.com/watch?v=LXb3EKWsInQ', clicks: 45 },
        { id: 2, type: 'file', category: 'file', title: 'Invoice & Kontrak.pdf', url: 'https://drive.google.com/', clicks: 12 },
        { id: 3, type: 'photo', category: 'photo', title: 'Link Folder Foto HD', url: 'https://drive.google.com/', clicks: 12 },
      ],
      submittedTestimonials: [
        { id: 1, name: 'Bpk. Budi (Alpha Corp)', message: 'Sangat puas dengan hasilnya! Pelayanan cepat dan responsif.', date: '10/02/2026' }
      ]
    }
  ]);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      const now = Date.now();
      setProjects((currentProjects) => currentProjects.filter(p => !p.expiresAt || p.expiresAt > now));
      setTick(t => t + 1);
    }, 1000);
  ...
  const handleAddItem = () => {
    ...
    const newItem = {
      id: Date.now(),
      type: addItemCategory === 'folder' ? 'folder' : addItemCategory,
  ...
  const handleExportCSV = () => {
    ...
    if (!activeProj || !activeProj.submittedTestimonials || activeProj.submittedTestimonials.length === 0) {
      showNotification('Tidak ada data testimoni untuk diekspor.');
      return;
    }
  ...
  const activeFolder = activeProject?.items.find(i => i.id === activeFolderId);
  const displayedItems = activeFolder ? activeFolder.items : (activeProject ? activeProject.items : []);
      ...
      <div className="min-h-screen flex items-center justify-center p-4 font-sans bg-white">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl border border-gray-100 p-8 relative overflow-hidden">
          ...
          <div className="text-center mb-8 pt-4">
            <img src={LOGO_URL} alt="Logo" className="w-20 h-20 rounded-full mx-auto shadow-md mb-4 border border-gray-100 object-cover" style={{ backgroundColor: COLOR_PRIMARY }} />
          ...
          <form onSubmit={(e) => {
            ...
            <Input label="Username" type="text" placeholder="admin" onChange={()=>{}} name="u"/>
            <Input label="Password" type="password" placeholder="123" onChange={()=>{}} name="p"/>
            {authError && <div className="text-red-600 text-sm text-center font-medium bg-red-50 py-2 rounded-lg">{authError}</div>}
            <Button variant="primary" className="w-full mt-2 py-3" type="submit">LOGIN</Button>
          </form>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gray-50 font-sans pb-12">
        <nav className="bg-white px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm border-b border-gray-200">
          <div className="flex items-center gap-3">
             <img src={LOGO_URL} className="w-10 h-10 rounded-full border border-gray-200 object-cover" style={{ backgroundColor: COLOR_PRIMARY }} alt="Logo" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {isCreatingProject && (
              <Card className="p-6 h-full border-dashed border-2 bg-slate-50 relative animate-fade-in" style={{ borderColor: COLOR_PRIMARY }}>
                ...
                <Input label="Nama Project / Klien" value={newProjectName} onChange={e => setNewProjectName(e.target.value)} placeholder="Contoh: Wedding A & B" />
                <Select 
                  label="Durasi Aktif"
  ...
  if (currentView === 'project-editor' && activeProject) {
    return (
      <div className="min-h-screen bg-gray-50 font-sans flex flex-col">
                ...
                {!isAddingItem ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-black uppercase tracking-widest text-sm flex items-center" style={{ color: COLOR_PRIMARY }}>
                         {addItemCategory === 'link' && <LinkIcon size={16} className="mr-2 text-blue-600"/>}
                         {addItemCategory === 'file' && <File size={16} className="mr-2 text-orange-600"/>}
                         {addItemCategory === 'photo' && <ImageIcon size={16} className="mr-2 text-purple-600"/>}
                         {addItemCategory === 'video' && <Video size={16} className="mr-2" style={{ color: COLOR_SECONDARY }}/>}
                         {addItemCategory === 'folder' && <Folder size={16} className="mr-2" style={{ color: COLOR_PRIMARY }}/>}
                         TAMBAH {addItemCategory}
                      </h3>
                      <button onClick={() => setIsAddingItem(false)} className="p-1 text-gray-400 hover:text-red-500 bg-gray-100 rounded-md"><X size={16}/></button>
                    </div>
                    ...
                    <Button variant="primary" className="w-full py-3" onClick={handleAddItem}>Simpan Item</Button>
                  </div>
                )}
              </Card>
                  ...
                  return (
                    <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm transition-all flex items-center justify-between group">
                      <div className="flex items-center gap-4 overflow-hidden">
                        <div className={`p-3 rounded-xl ${iconColor}`} style={item.type === 'video' ? {color: COLOR_SECONDARY} : item.type === 'folder' ? {color: COLOR_PRIMARY} : {}}>
                        ...
                        <div className="min-w-0">
                          ...
                          {item.type === 'video' && getYouTubeId(item.url) && <span className="text-[10px] text-white px-1.5 py-0.5 rounded ml-2 font-bold uppercase tracking-wider" style={{backgroundColor: COLOR_SECONDARY}}>YouTube</span>}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        }} className="p-2.5 text-gray-400 hover:text-blue-600 rounded-lg hover:bg-blue-50 transition-colors" title="Edit Item"><Edit3 size={18}/></button>
                        <button onClick={() => {
                           if(confirm("Hapus item ini secara permanen?")) {
                              setProjects(prev => prev.map(p => {
                                 if (p.id !== activeProjectId) return p;
                                 const filterFn = i => i.id !== item.id;
                                 if (!activeFolderId) return { ...p, items: p.items.filter(filterFn) };
                                 return { ...p, items: p.items.map(fol => fol.id === activeFolderId ? { ...fol, items: fol.items.filter(filterFn) } : fol) };
                              }));
                           }
                        }} className="p-2.5 text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50 transition-colors"><Trash2 size={18}/></button>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="grid grid-cols-1 gap-6 animate-fade-in">
              <Card className="p-6 shadow-md">
                 ...
                 <Input label="Judul Header" value={activeProject.config.title} onChange={e => {
                    setProjects(prev => prev.map(p => p.id === activeProjectId ? {...p, config: {...p.config, title: e.target.value}} : p));
                 }}/>
                 <TextArea label="Pesan Sambutan" value={activeProject.config.description} onChange={e => {
                     setProjects(prev => prev.map(p => p.id === activeProjectId ? {...p, config: {...p.config, description: e.target.value}} : p));
                 }}/>
              </Card>
              ...
              <Card className="p-6 shadow-md">
                 ...
                 <Input label="Nomor WhatsApp (Cth: 628123...)" value={activeProject.config.whatsapp} placeholder="Kosongkan jika tidak ada" onChange={e => {
                     setProjects(prev => prev.map(p => p.id === activeProjectId ? {...p, config: {...p.config, whatsapp: e.target.value}} : p));
                 }}/>
                 <div className="mt-6">
                   <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl mb-4">
                     <p className="text-xs text-blue-800 font-medium">Aktifkan fitur ini jika Anda ingin klien mengisi form ulasan (testimoni) saat pertama kali membuka link project ini.</p>
                   </div>
                   <Select 
                     ...
                     onChange={e => {
                         ...
                         setProjects(prev => prev.map(p => p.id === activeProjectId ? {...p, config: {...p.config, enableTestimonialForm: val}} : p));
                     }}
                     options={[
            ...
            <div className="animate-fade-in">
               <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                  <div>
                     ...
                     <p className="text-sm text-gray-500">Ulasan yang masuk untuk project ini.</p>
                  </div>
               ...
               <div className="space-y-4">
                  {(!activeProject.submittedTestimonials || activeProject.submittedTestimonials.length === 0) ? (
                     <div className="text-center py-12 bg-white rounded-xl border border-dashed border-gray-300 text-gray-500 font-medium">Belum ada testimoni untuk project ini.</div>
                  ) : (
                     activeProject.submittedTestimonials.map(t => (
                        <Card key={t.id} className="p-5 border-l-4 shadow-sm hover:shadow-md transition-shadow" style={{borderLeftColor: COLOR_TERTIARY}}>
                           <div className="flex justify-between items-start mb-2">
                              <h4 className="font-bold" style={{ color: COLOR_PRIMARY }}>{t.name}</h4>
        ...
        {notification && <div className="fixed bottom-4 right-4 text-white px-5 py-3 rounded-lg shadow-xl flex items-center z-50 text-sm font-bold tracking-wide" style={{ backgroundColor: COLOR_PRIMARY }}><Check size={18} className="mr-2" style={{ color: COLOR_TERTIARY }}/> {notification}</div>}
      </div>
    );
  }
  ...
  if (currentView === 'client-view' && activeProject) {
    ...
    const ClientItemRenderer = ({ item }) => {
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-4 shadow-sm w-full">
            ...
            {isOpen && (
              <div className="bg-slate-50 p-4 space-y-3 border-t border-gray-200">
                {item.items.length === 0 && <p className="text-sm text-slate-400 text-center py-4 font-medium">Folder ini kosong</p>}
      ...
      if (youtubeId) {
        return (
          <div className="mb-6 bg-white p-4 rounded-xl shadow-md hover:shadow-lg transition-shadow w-full" style={{ border: `1px solid rgba(1, 1, 61, 0.15)` }}>
             ...
             <div className="w-full aspect-video rounded-xl overflow-hidden shadow-inner" style={{ backgroundColor: COLOR_PRIMARY }}>
               <iframe
      ...
      if (item.type === 'photo') { IconVar = ImageIcon; iconColor = "text-purple-600 bg-purple-50"; }
      else if (item.type === 'file') { IconVar = File; iconColor = "text-orange-600 bg-orange-50"; }
      else if (item.type === 'video') { IconVar = Video; iconColor = `text-[${COLOR_SECONDARY}] bg-pink-50`; }

      return (
        <a 
          href={item.url} target="_blank" rel="noreferrer"
          className="block bg-white p-5 rounded-xl shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 mb-4 group relative overflow-hidden w-full"
          style={{ border: `1px solid rgba(1, 1, 61, 0.15)` }}
        >
          <div className="flex items-center justify-between relative z-10 w-full">
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-xl ${iconColor} transition-transform duration-300 group-hover:scale-110`} style={item.type === 'video' ? {color: COLOR_SECONDARY} : {}}>
                <IconVar size={22} />
              </div>
            ...
            <div className={`w-10 h-10 rounded-full flex items-center justify-center bg-slate-50 text-slate-400 group-hover:text-white transition-all duration-300 shadow-sm shrink-0`} style={{ '--tw-group-hover-bg': COLOR_PRIMARY }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = COLOR_PRIMARY} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#f8fafc'}>
    ...
    return (
      <div className="min-h-screen bg-slate-100 font-sans flex flex-col relative">
        {/* Modal Form Testimonial Klien (Muncul 1 Kali) */}
        {showTestimonial && activeProject.config.enableTestimonialForm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md animate-fade-in">
             <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center relative shadow-2xl">
                ...
                }} className="absolute top-5 right-5 text-gray-400 hover:text-gray-800 bg-gray-50 p-1 rounded-full"><X size={20}/></button>
                
                <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-5 shadow-inner" style={{ backgroundColor: 'rgba(1, 1, 61, 0.05)' }}>
                  <MessageSquare size={28} style={{ color: COLOR_PRIMARY }} />
                </div>
                
                <h3 className="font-black text-xl mb-2 uppercase tracking-wide" style={{ color: COLOR_PRIMARY }}>Berikan Ulasan</h3>
                <p className="text-slate-500 text-xs mb-6 font-medium leading-relaxed">Bagaimana pengalaman Anda bekerja sama dengan One Dream Creative?</p>
                
                <div className="space-y-4 text-left">
                  <Input 
                    label="Nama / Instansi" 
                    placeholder="Ketik nama Anda..." 
                    value={clientTestiName} 
                    onChange={e => setClientTestiName(e.target.value)} 
                  />
                  <TextArea 
                    label="Pesan & Testimoni" 
                    placeholder="Tuliskan ulasan Anda mengenai hasil project ini..." 
                    value={clientTestiMsg} 
                    onChange={e => setClientTestiMsg(e.target.value)} 
                    rows={3}
                  />
                  <Button variant="primary" className="w-full mt-2 py-3" onClick={() => {
                      if (!clientTestiName || !clientTestiMsg) {
                          alert("Harap isi nama dan pesan testimoni Anda.");
                          return;
                      }
                      
                      // Simpan testimoni ke state project
                      setProjects(prev => prev.map(p => {
                        if (p.id === activeProject.id) {
                          const newTesti = {
                            id: Date.now(),
                            name: clientTestiName,
                            message: clientTestiMsg,
                            date: new Date().toLocaleDateString('id-ID')
                          };
                          return { ...p, submittedTestimonials: [...(p.submittedTestimonials || []), newTesti] };
                        }
                        return p;
                      }));
                      ...
                      setShowTestimonial(false);
                      localStorage.setItem(`testi_shown_${activeProject.id}`, 'true');
                      alert("Terima kasih! Testimoni Anda telah kami terima.");
                      setClientTestiName('');
                      setClientTestiMsg('');
                  }}>Kirim Ulasan</Button>
                </div>
        ...
        <div className="fixed top-4 left-4 z-50">
           <button onClick={() => setCurrentView('project-editor')} className="bg-white/80 backdrop-blur text-slate-800 p-2 rounded-full shadow border hover:bg-white"><ArrowLeft size={18}/></button>
        </div>

        {/* HEADER AREA */}
        <div 
          className="relative text-white pt-24 pb-24 px-4 text-center bg-cover bg-center"
          style={{ backgroundImage: `url('${HEADER_BG_URL}')`, borderBottom: `4px solid ${COLOR_PRIMARY}` }}
        >
          {/* Overlay Gelap agar teks terbaca */}
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm" style={{ backgroundColor: 'rgba(1, 1, 61, 0.8)' }}></div> 
          
          <div className="relative z-10 max-w-md mx-auto flex flex-col items-center w-full">
             <img src={LOGO_URL} alt="One Dream Creative" className="w-24 h-24 rounded-full object-cover shadow-2xl mb-6 bg-white" style={{ border: `4px solid ${COLOR_PRIMARY}` }} />
             <h1 className="text-3xl font-black mb-4 leading-tight uppercase tracking-tight w-full break-words px-2">{activeProject.config.title}</h1>
             <p className="text-slate-200 text-sm leading-relaxed font-medium w-full px-2 break-words">{activeProject.config.description}</p>
          </div>
        </div>

        {/* AREA KONTEN (MAX-W-MD ALIGNMENT) */}
        <div className="flex-1 max-w-md w-full mx-auto px-4 relative z-20 pb-10">
          
          {/* BADGE COUNTDOWN (Warna Sekunder, Teks Putih, Tanpa Border) */}
          <div className="shadow-xl rounded-xl p-4 flex items-center justify-between gap-4 -mt-10 mb-6" style={{ backgroundColor: COLOR_SECONDARY }}>
              <div className="flex flex-col text-left w-full text-white">
                <span className="text-[10px] font-bold tracking-widest uppercase mb-1 opacity-90">Batas Waktu Akses & Download</span>
                <span className="text-sm font-black">{formatTimeLeft(activeProject.expiresAt)}</span>
              </div>
              <div className="p-3 rounded-lg shrink-0" style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)' }}>
        ...
        {/* WHATSAPP BUTTON (Static, Above Footer) */}
        {activeProject.config.whatsapp && (
           <div className="px-6 py-10 flex justify-center bg-transparent relative z-20">
             <a 
               href={`https://wa.me/${activeProject.config.whatsapp}`}
               target="_blank" rel="noreferrer"
               className="bg-green-600 hover:bg-green-700 text-white px-8 py-4 rounded-full shadow-lg hover:shadow-xl flex items-center gap-3 text-sm font-bold uppercase tracking-wider transition-all duration-300 hover:scale-105 border border-green-500"
             >
               <MessageCircle size={20}/> Jika ada kendala, hubungi kami
             </a>
           </div>
        )}

        {/* FOOTER */}
        <footer className="bg-white font-bold text-center py-8 text-[10px] uppercase tracking-widest border-t border-gray-200" style={{ color: COLOR_PRIMARY }}>
          <p>© {new Date().getFullYear()} PT ONE DREAM CREATIVE. ALL RIGHTS RESERVED.</p>
        </footer>
      </div>
    );
  }
